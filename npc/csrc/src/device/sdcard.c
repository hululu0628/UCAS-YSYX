...
static void sdcard_handle_cmd(int cmd) {
  switch (cmd) {
    ...
    default:
      Log_Error("unhandled command = " << cmd);
  }
}
...
static void sdcard_io_handler(uint32_t offset, int len, bool is_write) {
  ...
  switch (idx) {
    ...
    case SDDATA:
       ...
       break;
    default:
      Log("offset = 0x" << std::hex << offset << "(idx = " << idx << "), is_write = " << is_write << ", data = 0x" << base[idx]);
      Log_Error("unhandle offset = " << offset);
  }
}
...
void init_sdcard() {
  ...
  fp = fopen(img, "r+");
  if (fp == NULL) Log("Can not find sdcard image: " << img);
}
